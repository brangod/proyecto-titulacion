<x-guest-layout>
    <x-jet-authentication-card>
        <x-slot name="logo">
            <div class="w-full bg-white border-2 rounded-xl">
                <a href="{{route('index')}}">
                    <img src="{{ asset('img/logo.jpg') }}" alt="logo" class="rounded-xl">
                </a>
            </div>
        </x-slot>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="mt-4">
                <x-jet-label for="boleta" value="{{ __('Boleta') }}" />
                <x-jet-input id="boleta" class="block w-full mt-1" type="number" name="boleta" :value="old('boleta')" required />
            </div>
